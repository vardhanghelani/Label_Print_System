import type { InterlockSheetGeometry } from './geometryBuilder.js';
import { buildInterlockStickers } from './geometryBuilder.js';
import type { StickerDefinition } from './index.js';
...
export function validateInterlockGeometry(
  ...
  for (const sticker of stickers) {
    ...
    if (Math.abs(tailCenterY - broadCenterY) > geometry.broadHeight / 2 + 0.01) {
      issues.push({
        code: 'TAIL_DETACHED',
